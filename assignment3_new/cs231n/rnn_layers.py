...
def rnn_step_backward(dnext_h, cache):
    ...
    df = dnext_h * (1 - next_h**2) # shape (N, H)
    dx = np.dot(df, Wx.T)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (i, f, o, g, prev_c, next_c, Wx, Wh, x, prev_h)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    i, f, o, g, prev_c, next_c, Wx, Wh, x, prev_h = cache
    
    dc = dnext_h * o * (1 - np.tanh(next_c)**2) 
    do = dnext_h * np.tanh(next_c)
    
    dprev_c = (dnext_c + dc) * f
    di = (dnext_c + dc) * g 
    df = (dnext_c + dc) * prev_c
    dg = (dnext_c + dc) * i
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        h[:, t, :], next_c, step_cache = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b)
        cache.append(step_cache)
        prev_h = h[:, t, :]
        prev_c = next_c
    ##############################################################################
